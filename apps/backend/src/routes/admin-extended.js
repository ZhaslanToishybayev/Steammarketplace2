...
router.get('/content', async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT id, page, section, content_type, content, is_active, updated_at
            FROM site_content
...
router.post('/content', async (req, res) => {
    const { page, section, content, content_type = 'text' } = req.body;

    if (!page || !section) {
        return res.status(400).json({ success: false, error: 'Page and section are required' });
    }

    try {
        const result = await pool.query(`
            INSERT INTO site_content (page, section, content, content_type)
            VALUES ($1, $2, $3, $4)
            RETURNING *
        `, [page, section, content || '', content_type]);
...
router.post('/banners', async (req, res) => {
    const { title, image_url, link_url, position = 'home_top', display_order = 0 } = req.body;
    ...
    try {
        const result = await pool.query(`
            INSERT INTO banners (title, image_url, link_url, position, display_order)
            VALUES ($1, $2, $3, $4, $5)
            RETURNING *
...
router.put('/banners/:id', async (req, res) => {
    const { id } = req.params;
    const { title, image_url, link_url, position, is_active, display_order } = req.body;
