...
const adminAuth = async (req, res, next) => {
    ...
    try {
        ...
        const result = await pool.query(
            'SELECT id, username, role, is_active, steam_id FROM admins WHERE id = $1',
            [decoded.adminId]
        );
        ...
        if (!result.rows[0].role || result.rows[0].role === 'inactive') {
...
router.get('/audit-logs', adminAuth, async (req, res) => {
    res.json({ success: true, data: { logs: [], pagination: { total: 0 } } });
});
...
router.get('/users', adminAuth, async (req, res) => {
    ...
    try {
        let query = `
            SELECT steam_id, username, avatar, balance, is_banned, created_at
            FROM users
        `;
...
router.patch('/users/:id', adminAuth, async (req, res) => {
    ...
    try {
        ...
        params.push(id);
        const result = await pool.query(
            `UPDATE users SET ${updates.join(', ')} WHERE steam_id = $${paramIndex} RETURNING *`,
            params
        );
...
router.get('/listings', adminAuth, async (req, res) => {
    ...
    try {
        let query = `
            ...
            FROM listings l
            LEFT JOIN users u ON l.seller_steam_id = u.steam_id
        `;
