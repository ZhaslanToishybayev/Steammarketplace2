...
class BotSessionService {
    constructor(redisUrl) {
        ...
        this.maxSessionAgeHours = 6; // Force re-login after 6 hours
        this.loginQueue = [];
    ...
    async getSession(accountName) {
        ...
        try {
            ...
            // Check if session is too old (force re-login every 6 hours)
            const sessionAgeHours = (Date.now() - session.savedAt) / (1000 * 60 * 60);
            if (sessionAgeHours > this.maxSessionAgeHours) {
                console.log(`[Session] Session too old (${sessionAgeHours.toFixed(1)} hours) for ${accountName}, forcing re-login`);
                await this.clearSession(accountName);
                return null;
            }

            console.log(`[Session] Found valid session for ${accountName} (age: ${sessionAgeHours.toFixed(1)} hours)`);
            return session;
        } catch (err) {
    ...
    /**
     * Mark session as invalid (called when Steam rejects cookies)
     */
    async markSessionInvalid(accountName, reason = 'Unknown') {
        console.warn(`[Session] Marking session as invalid for ${accountName}: ${reason}`);
        await this.clearSession(accountName);
    }
     ...
     */
    async getSessionAgeHours(accountName) {
        const data = await this.redis.get(this.getSessionKey(accountName));
        if (!data) return null;
