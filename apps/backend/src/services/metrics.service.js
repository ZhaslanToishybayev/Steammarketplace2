...
// 13. Bot Session Age (in hours)
const botSessionAge = new client.Gauge({
    name: 'bot_session_age_hours',
    help: 'Age of bot session in hours',
    labelNames: ['bot_name'],
});
register.registerMetric(botSessionAge);
...
register.registerMetric(retrySuccessTotal);
...
// 19. DLQ Pattern Detected
const dlqPatternDetected = new client.Counter({
    name: 'dlq_pattern_detected_total',
    help: 'Total error patterns detected in DLQ',
    labelNames: ['pattern_type'],
});
register.registerMetric(dlqPatternDetected);

// 20. Remediation Executed
const remediationExecuted = new client.Counter({
    name: 'remediation_executed_total',
    help: 'Total remediation actions executed',
    labelNames: ['action_type', 'result'],
});
register.registerMetric(remediationExecuted);

// ============== SMART BOT SELECTION METRICS ==============

// 21. Bot Selection Score
const botSelectionScore = new client.Gauge({
    name: 'bot_selection_score',
    help: 'Score assigned to bot during selection',
    labelNames: ['bot_name'],
});
register.registerMetric(botSelectionScore);

// 22. Bot Selection Count
const botSelectionCount = new client.Counter({
    name: 'bot_selection_count_total',
    help: 'Number of times bot was selected',
    labelNames: ['bot_name'],
});
register.registerMetric(botSelectionCount);
...
/**
 * Update bot session age metric
 * @param {string} botName - Bot account name
 * @param {number} ageHours - Session age in hours
 */
function updateBotSessionAge(botName, ageHours) {
    botSessionAge.labels(botName).set(ageHours);
}

/**
 * Update circuit breaker state metric
 * @param {string} service - Service name
 * @param {string} state - Circuit state ('CLOSED', 'HALF_OPEN', 'OPEN')
 */
...
function recordCircuitBreakerFailure(service) {
    circuitBreakerFailures.labels(service).inc();
}

/**
 * Record retry attempt
 * @param {string} errorType - Type of error (SESSION_ERROR, RATE_LIMIT, etc.)
 * @param {string} strategy - Retry strategy used
 */
function recordRetryAttempt(errorType, strategy) {
    retryAttemptsTotal.labels(errorType, strategy).inc();
}

/**
 * Record successful retry
 * @param {string} errorType - Type of error that was successfully retried
 */
function recordRetrySuccess(errorType) {
    retrySuccessTotal.labels(errorType).inc();
}

/**
 * Record DLQ job
 * @param {string} errorType - Type of error that caused job to fail
 */
function recordDLQJob(errorType) {
    dlqJobsTotal.labels(errorType).inc();
}

/**
 * Record DLQ pattern detection
 * @param {string} patternType - Type of pattern detected
 */
function recordDLQPattern(patternType) {
    dlqPatternDetected.labels(patternType).inc();
}

/**
 * Record remediation execution
 * @param {string} actionType - Type of remediation action
 * @param {string} result - Result of remediation (success/failure)
 */
function recordRemediation(actionType, result) {
    remediationExecuted.labels(actionType, result).inc();
}

/**
 * Record bot selection score
 * @param {string} botName - Bot account name
 * @param {number} score - Selection score
 */
function recordBotSelectionScore(botName, score) {
    botSelectionScore.labels(botName).set(score);
}

/**
 * Record bot selection
 * @param {string} botName - Selected bot account name
 */
function recordBotSelection(botName) {
    botSelectionCount.labels(botName).inc();
}
