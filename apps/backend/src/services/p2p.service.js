...
class P2PService {
    ...
    async registerApiKey(steamId, apiKey) {
        ...
        try {
            await client.query(`
                INSERT INTO user_api_keys (steam_id, api_key_encrypted, iv, updated_at)
                VALUES ($1, $2, $3, NOW())
                ON CONFLICT (steam_id) 
                DO UPDATE SET api_key_encrypted = $2, iv = $3, updated_at = NOW()
            `, [steamId, encrypted.content, encrypted.iv]);

            // Store API key hash for scam protection (change detection)
            const scamProtection = require('./scam-protection.service');
            await scamProtection.storeApiKeyHash(steamId, apiKey);
    ...
    async getApiKey(steamId) {
        const res = await pool.query('SELECT api_key_encrypted, iv FROM user_api_keys WHERE steam_id = $1', [steamId]);
        if (res.rows.length === 0) return null;

        const { api_key_encrypted, iv } = res.rows[0];
        return decrypt({ content: api_key_encrypted, iv });
    }
