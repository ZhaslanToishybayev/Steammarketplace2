...
export default function SellPage() {
    ...
    const [sellMode, setSellMode] = useState<'listing' | 'instant'>('listing');
            ...
            <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950">
                <div className="min-h-[60vh] flex items-center justify-center p-4">
                    <motion.div
                        ...
                        className="bg-card backdrop-blur-2xl rounded-3xl p-10 text-center border border-border shadow-2xl max-w-md w-full"
                    >
                ...
                <main className="container mx-auto px-6 py-8">
                    ...
                    >
                        <h1 className="text-5xl font-bold text-white mb-4">
                            Sell Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">Skins</span>
                        </h1>
                        <p className="text-gray-400 text-lg max-w-2xl mx-auto">
                            List your CS2 skins for sale. Set your price, and earn money when other players buy them.
                        </p>
                    </motion.div>
                    ...
                    <Tabs defaultValue="sell" className="w-full">
                        <TabsList className="bg-white/5 border border-white/10 p-1 rounded-2xl mx-auto w-fit mb-10">
                            <TabsTrigger value="sell" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-purple-500 data-[state=active]:to-pink-500 data-[state=active]:text-white rounded-xl px-8 py-3 font-semibold">
                                📦 Sell Item
                            </TabsTrigger>
                        ...
                        <TabsContent value="sell">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                                ...
                                                {inventory.map((item, index) => (
                                                    <motion.div
                                                        ...
                                                        className={`cursor-pointer rounded-2xl p-3 transition-all duration-200 ${selectedItem?.assetid === item.assetid
                                                            ? 'bg-gradient-to-br from-purple-500/30 to-pink-500/30 border-2 border-purple-400 shadow-lg shadow-purple-500/20'
                                ...
                                >
                                    <h2 className="text-2xl font-bold text-white mb-6">Sell Item</h2>

                                    {/* Sell Mode Toggle */}
                                    <div className="bg-black/20 p-1 rounded-xl flex mb-6">
                                        <button
                                            onClick={() => setSellMode('listing')}
                                            className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${sellMode === 'listing' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'
                                                }`}
                                        >
                                            List for Sale (P2P)
                                        </button>
                                        <button
                                            ...
                                            className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${sellMode === 'instant' ? 'bg-amber-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'
                                                }`}
                                        >
                                            ⚡ Instant Sell
                                        </button>
                                    ...
                                    <AnimatePresence mode="wait">
                                        {selectedItem ? (
                                            ...
                                            >
                                                ...
                                                <div className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-2xl p-5 flex items-center gap-5 border border-purple-500/20">
                                                ...
                                                {sellMode === 'listing' ? (
                                                    <div className="space-y-3">
                                                        <label className="text-white font-medium">Your Price</label>
                                                        <div className="relative">
                                                            ...
                                                            <Input
                                                                ...
                                                                placeholder="0.00"
                                                                className="pl-8 h-14 text-xl bg-white/5 border-white/10 text-white placeholder:text-gray-500 rounded-xl focus:border-purple-500 focus:ring-purple-500/20"
                                                                required
                                                            />
                                                        </div>
                                                        <p className="text-gray-500 text-sm">You will receive: ${(parseFloat(price || '0') * 0.95).toFixed(2)} (5% fee)</p>
                                                    </div>
                                                ) : (
                                                    <div className="space-y-3 bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
                                                        <div className="flex justify-between items-center">
                                                            <span className="text-amber-200 font-medium">Instant Offer</span>
                                                            <span className="text-2xl font-bold text-amber-500">~${instantPriceEstimate}*</span>
                                                        </div>
                                                        <p className="text-xs text-amber-200/60">*Estimated. Actual price determined by bot.</p>
                                                        <p className="text-sm text-gray-300 mt-2">
                                                            The bot will send you a trade offer <b>immediately</b>.
                                                        </p>
                                                    </div>
                                                )}
                                                ...
                                                >
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-16 h-16 bg-black/30 rounded-xl overflow-hidden">
                                                            <img
                                                                src={`https://steamcommunity-a.akamaihd.net/economy/image/${listing.item_icon_url}`}
