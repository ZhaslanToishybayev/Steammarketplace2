...
import { ShoppingCart, ExternalLink, Sparkles, ShieldCheck, UserRound } from 'lucide-react';
...
export function ItemCard({ item: listing }: { item: MarketListing }) {
    ...
    const listingMeta = listing as any;
    const isP2P = listingMeta.listingType === 'p2p' || listingMeta.source === 'p2p';
    const lastVerifiedAt = listingMeta.lastVerifiedAt ? new Date(listingMeta.lastVerifiedAt) : null;
    const verifiedLabel = lastVerifiedAt
        ? `Verified ${lastVerifiedAt.toLocaleString()}`
        : (isP2P ? 'Verified before listing' : 'Bot inventory');
    ...
    const handleBuy = async (e: React.MouseEvent) => {
        ...
        const confirmed = window.confirm(`Buy ${itemName} for $${Number(itemPrice).toFixed(2)}?`);
        if (!confirmed) return;

        try {
            setIsBuying(true);
            const tradeUrl = window.prompt('Steam Trade URL', '') || '';
            if (isP2P && !tradeUrl) {
                toast.error('Steam Trade URL is required for P2P purchase');
                setIsBuying(false);
                return;
            }
            const res = isP2P
                ? await apiClient.p2p.buyListing(listing.id, tradeUrl)
                : await apiClient.escrow.buyListing(listing.id, tradeUrl);
            setIsBuying(false);

            if (res.success) {
                toast.success(res.message || (isP2P ? 'Funds reserved. Waiting for seller.' : 'Purchase successful!'));
                const uuid = res.tradeUuid || (res as any).data?.trade_uuid;
                if (uuid) {
                    router.push(`/trade/${uuid}`);
                }
            }
        } catch (error: any) {
            setIsBuying(false);
        ...
        >
            ...
            >
                ...
                <div className="aspect-square relative bg-gradient-to-br from-white/5 to-transparent p-4">
                    ...
                    <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-1 rounded-lg bg-black/60 backdrop-blur-sm text-xs font-bold text-white">
                        {isP2P ? <UserRound className="w-3.5 h-3.5 text-cyan-300" /> : <ShieldCheck className="w-3.5 h-3.5 text-emerald-300" />}
                        {isP2P ? 'P2P' : 'Bot'}
                    </div>
                ...
                <div className="p-4">
                    ...
                    <div className="text-xs text-gray-500 mb-3 truncate" title={verifiedLabel}>
                        {verifiedLabel}
                    </div>
