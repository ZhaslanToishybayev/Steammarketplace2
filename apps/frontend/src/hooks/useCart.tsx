...
export function CartProvider({ children }: { children: ReactNode }) {
    ...
    const refreshCart = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const res = await apiClient.fetchWithAuth('/api/cart');
            if (res.ok) {
    ...
    const addToCart = async (item: any) => {
        ...
        try {
            const res = await apiClient.fetchWithAuth('/api/cart/add', {
                method: 'POST',
    ...
    const removeFromCart = async (listingId: number) => {
        try {
            const res = await apiClient.fetchWithAuth(`/api/cart/${listingId}`, {
                method: 'DELETE'
            });
            if (res.ok) {
                refreshCart();
