...
export function useWallet() {
  ...
  const depositMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (response.data.balance !== undefined) {
        useAuthStore.getState().updateBalance(response.data.balance);
      }
  ...
  const total = transactionsData?.pages?.[0]?.total || 0;
  const pages = transactionsData?.pages?.[0]?.totalPages || 0;

  // Current balance (prefer real-time from auth store, fallback to API)
  const balance = userBalance !== undefined ? userBalance : (balanceData?.data?.balance || 0);
  const availableBalance = balanceData?.data?.availableBalance || balance;
  const pendingBalance = balanceData?.data?.pendingBalance || 0;
