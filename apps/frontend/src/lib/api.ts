...
class ApiClient {
  ...
  async createListing(data: {
    ...
    return this.p2p.createListing({
      ...
      itemName: data.marketHashName || data.name,
      image: data.iconUrl,
      appId: 730,
    });
  }
  ...
  async getMyListings(): Promise<{ success: boolean; listings: any[] }> {
    return this.request<{ success: boolean; listings: any[] }>('/p2p/my-listings');
  }

  // Cancel Listing
  async cancelListing(listingId: number): Promise<{ success: boolean }> {
    return this.p2p.cancelListing(listingId);
  }
  ...
  public escrow = {
    getListings: async (filters: any = {}) => {
      ...
      return this.request<{ success: boolean; data: any[] }>(`/escrow/listings${queryString ? `?${queryString}` : ''}`);
    },
    buyListing: async (id: string, tradeUrl?: string) => {
      return this.request<{ success: boolean; data: any; message?: string; tradeUuid?: string }>(`/escrow/buy/${id}`, {
        method: 'POST',
        body: JSON.stringify(tradeUrl ? { tradeUrl } : {})
      });
    },
    // NEW: Buy Cart (Batch)
  ...
  public p2p = {
    getListings: async (filters: any = {}) => {
      const params = new URLSearchParams(filters);
      const queryString = params.toString();
      return this.request<{ success: boolean; data: any[]; pagination?: any }>(`/p2p/listings${queryString ? `?${queryString}` : ''}`);
    },
    getMyListings: async () => {
      return this.request<{ success: boolean; listings: any[]; data: any[] }>('/p2p/my-listings');
    },
    createListing: async (data: { assetId: string; price: number; tradeUrl: string; itemName?: string; image?: string; appId?: number }) => {
      return this.request<{ success: boolean; listingId?: number; status?: string; listing?: any }>('/p2p/list-item', {
        method: 'POST',
        body: JSON.stringify(data)
      });
    },
    buyListing: async (listingId: number | string, tradeUrl: string) => {
      return this.request<{ success: boolean; tradeUuid?: string; status?: string; message?: string }>(`/p2p/buy/${listingId}`, {
        method: 'POST',
        body: JSON.stringify({ tradeUrl })
      });
    },
    markSellerSent: async (tradeUuid: string, tradeOfferId?: string) => {
      return this.request<{ success: boolean; status: string }>(`/p2p/trades/${tradeUuid}/seller-sent`, {
        method: 'POST',
        body: JSON.stringify({ tradeOfferId })
      });
    },
    cancelListing: async (listingId: number) => {
      return this.request<{ success: boolean }>(`/p2p/listings/${listingId}`, {
        method: 'DELETE'
      });
    },
    getSales: async () => {
    ...
    hasKey: async () => {
      return this.request<{ success: boolean; hasKey: boolean }>('/p2p/has-key');
    },
    getTrades: async (params?: Record<string, string>) => {
      const qs = params ? `?${new URLSearchParams(params)}` : '';
      return this.request<{ success: boolean; data: any[] }>(`/p2p/trades${qs}`);
    },
    getTrade: async (tradeUuid: string) => {
      return this.request<{ success: boolean; data: any }>(`/p2p/trades/${tradeUuid}`);
    },
    cancelTrade: async (tradeUuid: string, reason?: string) => {
      return this.request<{ success: boolean; status: string; message: string }>(`/p2p/trades/${tradeUuid}/cancel`, {
        method: 'POST',
        body: JSON.stringify({ reason })
      });
    },
    syncTrade: async (tradeUuid: string) => {
      return this.request<{ success: boolean; status: string; previousStatus: string }>(`/p2p/trades/${tradeUuid}/sync`, {
        method: 'POST'
      });
    }
  };
