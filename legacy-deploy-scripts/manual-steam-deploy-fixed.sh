...
# Create patch script content with correct path
...
# Write patch script directly to container in src directory
...
# Execute patch script from src directory
docker exec steam_backend sh -c 'cd src && node /tmp/apply-steam-patch.js'
