...
class RealSteamInventoryAccess {
  ...
  setupRoutes() {
    ...
    this.app.get('/api/inventory/my', async (req, res) => {
      try {
        ...
        const mySteamId = '76561198012345678'; // Твой SteamID
        ...
        if (inventory && inventory.length > 0) {
          ...
          res.json({
            success: false,
            error: 'Инвентарь пуст или недоступен',
  ...
  async initializeBot() {
    ...
    try {
      ...
      this.bot.on('loggedOn', (details) => {
        ...
        console.log(`🎮 Бот вошел в Steam как ${details.player_name}`);
  ...
  // Запуск системы
  async start(port = 3018) {
    console.log('🚀 Запуск Real Steam Inventory Access...');
    ...
    return new Promise((resolve) => {
      this.app.listen(port, () => {
        console.log(`🌐 Real Steam Inventory Access запущен на порту ${port}`);
        console.log('');
...
inventoryAccess.start(3018).catch(console.error);
