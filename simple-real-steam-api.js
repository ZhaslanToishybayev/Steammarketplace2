...
// УПРОЩЕННАЯ РЕАЛЬНАЯ STEAM API ИНТЕГРАЦИЯ
// Без проблемных зависимостей и маршрутов

const express = require('express');
const crypto = require('crypto');
const fetch = require('node-fetch');

class SimpleSteamAPIIntegration {
  constructor() {
    this.app = express();
  ...
  setupRoutes() {
    ...
    this.app.get('/api', (req, res) => {
      res.json({
        success: true,
        service: 'Simple Real Steam API Integration',
        version: '1.0.0',
        endpoints: {
          steam: {
    ...
    // Эндпоинт для тестирования реального Steam API
    this.app.get('/api/test/real-api', async (req, res) => {
      try {
        console.log('🧪 Тестирование реального Steam Web API...');

        // Тестовый запрос к Steam Web API
        const testResponse = await fetch(
          `https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/` +
          `?key=${this.steamApiKey}&steamids=${this.botSteamId}`
        );

        const testData = await testResponse.json();

        if (testData.response && testData.response.players && testData.response.players.length > 0) {
          console.log('✅ Реальный Steam Web API доступен и работает');
          res.json({
            success: true,
            data: {
              apiStatus: 'working',
              ...
              testPlayerName: testData.response.players[0].personaname,
              realSteamAPI: true,
              timestamp: new Date().toISOString()
            }
          });
        } else {
          console.log('❌ Реальный Steam Web API недоступен');
          res.json({
            success: false,
            error: 'Steam Web API недоступен',
            details: 'API key может быть недействительным или API временно недоступен'
          });
        }
      } catch (error) {
        console.error('❌ Ошибка тестирования Steam Web API:', error.message);
        res.status(500).json({
          success: false,
          error: 'Ошибка тестирования Steam Web API',
          details: error.message
        });
      }
    });
  ...
  async start(port = 3016) {
    console.log('🚀 Запуск Simple Real Steam API Integration...');
    ...
    this.app.listen(port, () => {
      console.log(`🌐 Simple Real Steam API Integration запущен на порту ${port}`);
      console.log('');
      ...
      console.log('   🧪 GET /api/test/real-api - Тест реального Steam API');
      console.log('');
      ...
      console.log('   🚀 Готов к реальной работе!');
    });
...
// Запуск системы
const steamAPIIntegration = new SimpleSteamAPIIntegration();
steamAPIIntegration.start(3016).catch(console.error);

module.exports = SimpleSteamAPIIntegration;