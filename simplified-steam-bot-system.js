...
// Упрощенная Steam Bot Trading System
// Работает с реальными Steam API и вашими учетными данными
...
class SimplifiedSteamBotSystem {
  constructor() {
    this.bots = new Map();
    this.app = express();
    ...
    this.app.get('*', (req, res) => {
      ...
      res.sendFile(path.join(__dirname, 'public', 'index.html'), (err) => {
        if (err) {
          ...
          res.json({
            success: true,
            service: 'Simplified Steam Bot Trading System',
            version: '1.0.0',
            endpoints: {
              bots: {
  ...
  async start(port = 3013) {
    ...
    this.app.listen(port, () => {
      console.log(`🚀 Simplified Steam Bot Trading System запущен на порту ${port}`);
...
// Запуск системы
const steamBotSystem = new SimplifiedSteamBotSystem();
steamBotSystem.start(3013).catch(console.error);

module.exports = SimplifiedSteamBotSystem;