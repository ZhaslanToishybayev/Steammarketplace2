...
const STEAM_REALM = process.env.STEAM_REALM || 'http://localhost:3008';
    ...
    try {
      ...
      if (queryParams['openid.mode'] === 'id_res') {
        ...
        if (steamId) {
          ...
          this.getSteamUserProfile(steamId).then(userProfile => {
            ...
            this.sendAuthError(res, 'Server error during authentication');
          });
          return;
        }
      ...
      // Handle error case
      this.sendAuthError(res, 'Authentication failed or cancelled');
