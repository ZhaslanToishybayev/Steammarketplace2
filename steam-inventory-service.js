        ...
        try {
          ...
          const items = inventory.assets.map(asset => {
            ...
            return {
              ...
              appId: appId,
              // В реальной системе цена должна получаться из Steam Market API
              price: Math.random() * 100 + 1 // Заглушка для демонстрации
            };
          });
...
const routes = {
  ...
  '/health': (req, res) => {
    ...
    res.end(JSON.stringify({
      status: 'healthy',
      service: 'steam-inventory-service',
  ...
  '/inventory/:steamId': (req, res) => {
    try {
      ...
      const steamId = pathParts[2]; // /inventory/:steamId
      const appId = parsedUrl.query.appId || '730'; // Default to CS2
  ...
  '/inventory/me': (req, res) => {
    try {
      ...
      // Просто перенаправляем на /inventory/:steamId
...
server.listen(PORT, () => {
  console.log(`🚀 Steam Inventory Service запущен на порту ${PORT}`);
  console.log(`📍 Health check: http://localhost:${PORT}/health`);
  console.log(`📦 Эндпоинты инвентаря доступны:`);
  console.log(`   - http://localhost:${PORT}/inventory/me?appId=730 (твой инвентарь)`);
  console.log(`   - http://localhost:${PORT}/inventory/:steamId?appId=730 (чужой инвентарь)`);
  ...
  console.log(`🎮 Используется настоящий Steam Community Inventory API`);
});
