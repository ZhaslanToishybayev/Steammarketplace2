...
const app = express();
const PORT = 3012; // Use different port to avoid conflicts and nginx proxy
...
app.get('/health', (req, res) => {
  res.json({
    ...
    service: 'steam-marketplace-api-test',
    version: '1.0.0',
    port: PORT
  });
});
...
app.get('/api/auth/steam', (req, res) => {
  res.json({
    ...
    data: {
      authUrl: `https://steamcommunity.com/openid/login?openid.ns=http://specs.openid.net/auth/2.0&openid.mode=checkid_setup&openid.return_to=http://localhost:${PORT}/api/auth/steam/return&openid.realm=http://localhost:${PORT}&openid.identity=http://specs.openid.net/auth/2.0/identifier_select&openid.claimed_id=http://specs.openid.net/auth/2.0/identifier_select`
    }
  });
});
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Steam Marketplace API Test running on port ${PORT}`);
