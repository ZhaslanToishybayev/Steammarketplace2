...
const server = http.createServer((req, res) => {
    ...
    // Serve Steam Marketplace for main routes
    if (!extname || !fs.existsSync(__dirname + filePath)) {
        if (parsedUrl.pathname.startsWith('/auth/') ||
            ...
            // OAuth callback - redirect to marketplace
            res.writeHead(302, { 'Location': '/marketplace' });
            res.end();
            return;
        }
        filePath = '/steam-marketplace.html';
    }
    ...
    fs.readFile(fullPath, (err, content) => {
        if (err) {
            if (err.code === 'ENOENT') {
                ...
                res.end('<h1>404 - Page Not Found</h1><p>Try <a href="/marketplace">Steam Marketplace</a></p>');
            } else {
                res.writeHead(500, { 'Content-Type': 'text/plain' });
                res.end('Server Error');
            }
...
server.listen(PORT, () => {
    ...
    console.log(`   • Trade: http://localhost:${PORT}/trade`);
